...
export class BankUploadService {
  ...
  private async finalizeUploadBatch(batchId: string, result: UploadResult): Promise<void> {
    ...
    // (same examId+year+shift on every row, paperCode optional — exactly what a
  ...
  // that (a) has examId+year+shift present on every row [a genuine single
  // paper, not a mixed bag — paperCode is optional, see BUGFIX comment
  // below] and (b) has at least MIN_PAPER_QUESTIONS questions [enough to
  // be worth a dedicated mock — avoids e.g. a 2-question test-upload
  // accidentally spawning a "mock"].
  //
  // Deterministic template id (`pyq-<slug of paperCode, or year-shift when
  // paperCode is blank>`) means re-uploading corrections/additions to the
  // same paper UPDATES the existing mock's question count/title instead of
  // creating a duplicate — same idea as this file's other
  // upsert-by-natural-key patterns (createUploadBatchPlaceholder, etc.)
  // rather than ever risking two mocks for one real paper.
  ...
  private async autoCreatePyqMocksForBatch(batchId: string): Promise<void> {
    ...
    for (const r of rows) {
      ...
      // are still required (a "complete real paper" needs at least these
      // three — a practice question with no year/shift genuinely isn't a
      // PYQ paper), but paperCode is no longer mandatory. Plenty of real
      // uploads have examId+year+shift filled in with paperCode left
      // blank (admin doesn't always have/know the official paper code) —
      // those used to be silently skipped here and NEVER got an auto-mock
      // at all. Rows sharing the same exam+year+shift with no paperCode
      // now group together under a null-paperCode bucket instead.
      if (!r.examId || !r.year || !r.shift) continue;
  ...
  // paperCode is null when a batch's rows all share examId+year+shift but
  // never had a real paperCode entered (see the BUGFIX comment on the
  // caller above) — falls back to a synthetic `<year>-<shift>` code for
  // the template id/title, while every DB query below still matches on
