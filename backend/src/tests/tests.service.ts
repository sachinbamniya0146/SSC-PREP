...
async saveAnswers(
  ...
  private async shiftWisePyqPaper(
    ...
    let rows = candidates.filter((r) => r.paperCode && this.slugifyPaperCode(r.paperCode) === wantedSlug);

    // BUGFIX (Sachin — "ek hee exam or ek he year me or ek he shift me
    // puche gye ho to automatic pyq bn jana chaiye"): BankUploadService
    // .upsertPyqMockForPaper() now also auto-creates a mock for rows that
    // share examId+year+shift with NO paperCode (falling back to a
    // synthetic `<year>-<shift>` code — see its own doc-comment). The
    // paperCode-based match above only ever finds real-paperCode papers,
    // so a synthetic-code template found zero rows here and threw "no
    // longer available" even though the questions genuinely exist. Fall
    // back to matching examId+year+shift among paperCode-IS-NULL questions
    // whenever the paperCode-based match comes up empty.
    if (rows.length === 0) {
      const nullPaperCandidates = await this.prisma.question.findMany({
        where: { ...PUBLISHED_QUESTION_WHERE, paperCode: null, examId: { not: null }, year: { not: null }, shift: { not: null } },
        include: {
          exam: { select: { name: true } },
          subject: { select: { name: true, slug: true } },
          chapter: { select: { name: true } },
      ...
      rows = nullPaperCandidates.filter(
        (r) => r.year && r.shift && this.slugifyPaperCode(`${r.year}-${r.shift}`) === wantedSlug,
      );
    }
    if (rows.length === 0) {
  ...
  async getWeakAreasPractice(
      ...
      include: {
        question: {
          select: {
            ...
            subject: { select: { name: true } },
            exam: { select: { name: true } },
            topic: { select: { name: true, nameHindi: true } },
          },
        },
      },
    });
    ...
    // answer glt hoga unke questions direct practice ke liye"): same
    // aggregation as chapterMap below, one level finer — by topicId instead
    // of chapterId — so the frontend can show/link straight into the exact
    // weak TOPIC (not just the chapter it lives in). Rows whose question
    // has no topicId (older/ungrouped questions) are simply skipped here;
    // they still count fully toward the chapter-level aggregation below.
    const topicMap = new Map<string, { topicId: string; topicName: string; topicNameHindi: string | null; chapterId: string; chapterName: string; subjectName: string; examName: string | null; wrongCount: number; skippedCount: number }>();
    for (const ws of wrongSkipped) {
      const q = ws.question;
      if (!q?.topicId || !q.chapterId) continue;
      const existing = topicMap.get(q.topicId) ?? {
        topicId: q.topicId,
        topicName: q.topic?.name ?? 'Unknown',
        topicNameHindi: q.topic?.nameHindi ?? null,
    ...
    const weakTopics = [...topicMap.values()]
      ...
      .slice(0, 20)
      .map((t) => ({ ...t, totalErrors: t.wrongCount + t.skippedCount }));
    ...
    return {
      ...
      // NEW — finer-grained sibling of `chapters` above; frontend uses
      // this to let a student jump directly into practice for one weak
      // TOPIC (via POST /bank/practice/start with topicId) instead of the
      // whole chapter. Free/unlimited for exactly these topics — see
      // QuestionBankPracticeService.isTopicWeakForUser().
      weakTopics,
