import { Controller, Get, Post, Param, Query, Res, HttpCode, UseGuards } from '@nestjs/common';
...
export class PdfExportController {
  ...
  // BUGFIX: premium templates now require ?token=<access token> on the URL —
  // see PdfExportService.download()'s doc comment for why. Free templates
  // are unaffected — no token needed, exactly as before.
  @Public()
  @Get('paper')
  async paper(@Param('testTemplateId') id: string, @Query('token') token: string | undefined, @Res() res: Response) {
    const { buffer, filename } = await this.service.download(id, 'paper', token);
  ...
  @Public()
  @Get('answerkey')
  async answerKey(@Param('testTemplateId') id: string, @Query('token') token: string | undefined, @Res() res: Response) {
    const { buffer, filename } = await this.service.download(id, 'answerkey', token);
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    res.send(buffer);
  }
